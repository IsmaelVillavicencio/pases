...
$("#nombre").change(function() {
	...
	if(!erroresGenerales) {
		$.ajax({
			url: base_url_rest + 'catalogos/transporte/nombre/' + nombre.value + '/0',
			type: 'GET',
			global: false,
			success: (data) => {
				if(data.data.length != 0) {
					...
					if(data.data.estatus == 0) {
						...
						localStorage.setItem("id_transporte", data.data.id)
					}
				}
...
$("#nombreCorto").change(function() {
	...
	if(!erroresGenerales) {
		$.ajax({
			url: base_url_rest + 'catalogos/transporte/nombrecorto/' + nombreCorto.value + '/0',
			type: 'GET',
			global: false,
			success: (data) => {
				if(data.data.length != 0) {
...
const ejecutar_reactivar = () => {
	...
	$.ajax({
		url: base_url_rest+'catalogos/transporte/en/'+id_transporte,
    type: "PUT",
    data: JSON.stringify({
			ip          : ip_address,
