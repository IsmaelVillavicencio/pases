...
const construir_datatable = (arr = []) => {
  if(U || D) {
    $('#tabRegistroEstadoTransporte').DataTable({
      destroy: true,
      stateSave: true,
      responsive: true,
      language: {
...
const obtener_registros = () => {
	$.ajax({
		url: base_url_rest + 'catalogos/estadotransporte',
		type: 'GET',
		success: (data) => {
			if(data.status != true)
...
const modificar = (id) => {
  localStorage.setItem("id_estado", id);
  let estado = estados.find(estado => estado.id == id);
  localStorage.setItem("nombre_estado", estado.nombre);
  window.location.href= base_url + "Catalogos/Ctrl_EstadosTransportes/modificar";
}
...
const ejecutar_baja = (id) => {
  $.ajax({
    url: base_url_rest + 'catalogos/estadotransporte/' + id,
    type: "DELETE",
    data: JSON.stringify({
      ip          : ip_address,
    ...
    success: (data) => {
      if(data.status == true) {
        ...
        let registro = estados.find(estado => estado.id == id),
          index = estados.indexOf(registro);
        estados[index].estatus = 0;
        construir_datatable(estados);
...
const ejecutar_reactivar = (id) => {
  $.ajax({
    url: base_url_rest + 'catalogos/estadotransporte/en/' + id,
    type: "PUT",
    data: JSON.stringify({
      ip          : ip_address,
      idusuario   : id_usuario_sesion
    ...
    success: (data) => {
      if(data.status == true) {
        let reactivar = estados.find(estado => estado.id == id),
          index = estados.indexOf(reactivar);
        estados[index].estatus = 1;
        construir_datatable(estados);
...
$("#confirmar_baja").click( (e) => {
  ejecutar_baja(localStorage.getItem("id_estado"));
});

$("#confirmar_reactivar_registro").click( (e) => {
  ejecutar_reactivar(localStorage.getItem("id_estado"));
});
