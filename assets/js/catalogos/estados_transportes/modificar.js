...
const id_estado = localStorage.getItem("id_estado"),
  _nombre_estado = localStorage.getItem("nombre_estado");
...
$(document).ready(() => {
  $('#nombre').val(_nombre_estado);
});

//FUNCIONES
const enviar_datos = ( obj ) => {
  $.ajax({
    url: base_url_rest+'catalogos/estadotransporte/'+id_estado,
    type: 'PUT',
    data: obj,
    beforeSend:function(){
...
$("#nombre").change(function() {
  ...
  if(!erroresGenerales) {
		$.ajax({
			url: base_url_rest + 'catalogos/estadotransporte/nombre/' + nombre.value + '/' + id_estado,
			type: 'GET',
			global: false,
			success: (data) => {
