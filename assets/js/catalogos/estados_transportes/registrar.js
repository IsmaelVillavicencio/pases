...
$("#nombre").change(function() {
	...
	if(!erroresGenerales) {
		$.ajax({
			url: base_url_rest + 'catalogos/estadotransporte/nombre/' + nombre.value + '/0',
			type: 'GET',
			global: false,
			success: (data) => {
				if(data.data.length != 0) {
					...
					if(data.data.estatus == 0) {
						...
						localStorage.setItem("id_estado", data.data.id)
					}
				}
...
const ejecutar_reactivar = () => {
	...
	$.ajax({
		url: base_url_rest+'catalogos/estadotransporte/en/'+id_estado,
    type: "PUT",
    data: JSON.stringify({
			ip          : ip_address,
...
$("#btnRegresar").click( (e) => {
	if(cambios == 1)
		...
		window.location.href = base_url + 'Catalogos/Ctrl_EstadosTransportes';
});
