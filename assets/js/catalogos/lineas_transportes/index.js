...
const construir_datatable = (arr = []) => {
  if(U || D) {
    $('#tabRegistroLineaTransporte').DataTable({
      destroy: true,
      stateSave: true,
      responsive: true,
      language: {
...
const modificar = (id) => {
  localStorage.setItem("id_linea", id);
  let linea = lineas.find(linea => linea.id == id);
  localStorage.setItem("nombre_linea", linea.nombre);
  window.location.href= base_url + "Catalogos/Ctrl_LineasTransportes/modificar";
}
...
const ejecutar_baja = (id) => {
  $.ajax({
    url: base_url_rest + 'catalogos/linea/' + id,
    type: "DELETE",
    data: JSON.stringify({
      ip          : ip_address,
    ...
    success: (data) => {
      if(data.status == true) {
        ...
        let registro = lineas.find(linea => linea.id == id),
          index = lineas.indexOf(registro);
        lineas[index].estatus = 0;
        construir_datatable(lineas);
...
const ejecutar_reactivar = (id) => {
  $.ajax({
    url: base_url_rest + 'catalogos/linea/en/' + id,
    type: "PUT",
    data: JSON.stringify({
      ip          : ip_address,
      idusuario   : id_usuario_sesion
    ...
    success: (data) => {
      if(data.status == true) {
        let reactivar = lineas.find(linea => linea.id == id),
          index = lineas.indexOf(reactivar);
        lineas[index].estatus = 1;
        construir_datatable(lineas);
...
$("#confirmar_baja").click( (e) => {
  ejecutar_baja(localStorage.getItem("id_linea"));
});

$("#confirmar_reactivar_registro").click( (e) => {
  ejecutar_reactivar(localStorage.getItem("id_linea"));
});
