...
const id_linea = localStorage.getItem("id_linea"),
  _nombre_linea = localStorage.getItem("nombre_linea");
...
$(document).ready(() => {
  $('#nombre').val(_nombre_linea);
});

//FUNCIONES
const enviar_datos = ( obj ) => {
  $.ajax({
    url: base_url_rest+'catalogos/linea/'+id_linea,
    type: 'PUT',
    data: obj,
    beforeSend:function(){
...
$("#nombre").change(function() {
  ...
  if(!erroresGenerales) {
		$.ajax({
			url: base_url_rest + 'catalogos/linea/nombre/' + nombre.value + '/' + id_linea,
			type: 'GET',
			global: false,
			success: (data) => {
